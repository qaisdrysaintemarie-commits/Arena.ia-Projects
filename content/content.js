...
(function () {
  ...
  // Configuration of selectors (adjust as needed for Arena.ai)
  const SELECTORS = {
  ...
  // UI configuration (texts and icons) – easy to modify!
  const UI_CONFIG = {
    titleNew: "New Project",
    titleUnfiled: "No Project",
    titleMove: "Move to...",
    titleRename: "Rename Project",
    promptDelete: 'Delete project "{name}"?\n\nConversations will be moved to "No Project".',
    placeholderNew: "New project...",
    placeholderName: "Project name...",
    btnCancel: "Cancel",
    btnCreate: "Create",
    btnSave: "Save",
    btnDelete: "Delete",
    tooltipMove: "Move to a project",
  ...
  let observer = null;
  let sidebarResizeObserver = null; // observe sidebar size changes
  let currentMenu = null;
  let draggedConversation = null;
  let foldersContainer = null;
  let conversationListElement = null; // store reference to original conversation list
  let originalConversationListDisplay = '';
  let _isInjecting = false; // re-entrancy guard for injectFolderInterface

  /**
   * Initialisation
   */
  function init() {
    console.log('🗂️ Arena.ai Conversation Folders: Initialising...');
  ...
  async function setup() {
    try {
      ...
      console.log('🗂️ Sidebar detected, injecting folders...');

      // Inject the folder UI
      await injectFolderInterface();

      // Observe SPA mutations
      setupMutationObserver();

      // Observe sidebar resize to re‑inject if it disappears
      setupSidebarResizeObserver();

      // Monitor sidebar presence to re-inject if sidebar is recreated
      monitorSidebarPresence();

      console.log('✅ Arena.ai Conversation Folders activated!');
    } catch (error) {
      console.error('🗂️ Arena.ai Conversation Folders: Initialization error (timeout). Attempting fallback...', error);
  ...
  function findConversationsList() {
    ...
    for (const selector of selectors) {
      try {
        const element = document.querySelector(selector.trim());
        // Skip if the element is inside our own injected UI
        if (element && !element.closest('.arena-folders-container')) return element;
      } catch (e) { }
    }
    return null;
  }
  ...
  /**
   * Trouver toutes les conversations (ONLY originals, never our clones)
   */
    ...
    for (const selector of selectors) {
      try {
        ...
        if (elements.length > 0) {
          // Filter out elements inside our own .arena-folders-container
          const originals = Array.from(elements).filter(
            el => !el.closest('.arena-folders-container')
          );
          if (originals.length > 0) {
            conversations.push(...originals);
            break;
          }
        }
      } catch (e) { }
  ...
  async function injectFolderInterface() {
    // Prevent re-entrant calls (our own DOM changes trigger observers)
    ...
    if (!conversationsList) {
      console.warn('⚠️ No conversation list found');
      _isInjecting = false;
      return;
    }
    ...
    // Store reference to conversation list and its original display
    conversationListElement = conversationsList;
    originalConversationListDisplay = conversationsList.style.display || '';

    // Hide the original conversation list so only our folder UI is visible
  ...
  // ── Sidebar re-injection logic ──
  // When Arena.ai collapses and re-expands the sidebar, it often
  // destroys the entire sidebar subtree and recreates it from scratch.
  // This means our injected .arena-folders-container is removed.
  // The strategy: use a persistent body-level MutationObserver to
  // detect when the container is gone and conversations reappear,
  // then re-inject.

  let _reinjectPending = false;

  function scheduleReinject() {
    if (_reinjectPending) return;
    _reinjectPending = true;
    // Use requestAnimationFrame + small delay to batch rapid DOM changes
    requestAnimationFrame(() => {
      setTimeout(async () => {
        _reinjectPending = false;
        const hasContainer = document.querySelector('.arena-folders-container');
        const conversations = findConversations();
        if (!hasContainer && conversations.length > 0) {
          console.log('🗂️ Container missing, re-injecting folders...');
          await injectFolderInterface();
          setupSidebarResizeObserver();
        }
      }, 300);
  ...
  function setupSidebarResizeObserver() {
    const attachObservers = () => {
      ...
      if (sidebarResizeObserver?.attrObserver) sidebarResizeObserver.attrObserver.disconnect();
      ...
      // Attribute observer for class/style toggles (collapsed/hidden)
      const attrObserver = new MutationObserver(() => {
        scheduleReinject();
      });
      attrObserver.observe(sidebar, { attributes: true, attributeFilter: ['class', 'style'] });
      // Store reference for cleanup
      sidebarResizeObserver.attrObserver = attrObserver;
      return true;
    };

    // Try to attach immediately; if sidebar not yet available, poll until it appears
    if (!attachObservers()) {
  ...
  // Monitor sidebar DOM: persistent observer that watches the entire body
  // for sidebar recreation (Arena.ai may destroy and rebuild the sidebar
  // subtree when toggling it). This is the KEY fix for the disappearing UI.
  function monitorSidebarPresence() {
    let _monitorDebounce = null;
    const bodyObserver = new MutationObserver((mutations) => {
      // Quick check: skip mutations caused by our own elements
      let isOwnMutation = true;
      for (const mutation of mutations) {
        const target = mutation.target;
        if (target.classList?.contains('arena-folders-container') ||
            target.closest?.('.arena-folders-container') ||
            target.classList?.contains('arena-move-menu') ||
            target.closest?.('.arena-move-menu') ||
            target.classList?.contains('arena-folder-modal-overlay') ||
            target.closest?.('.arena-folder-modal-overlay')) {
          continue;
        }
        isOwnMutation = false;
        break;
      ...
      clearTimeout(_monitorDebounce);
      _monitorDebounce = setTimeout(() => {
        const hasContainer = document.querySelector('.arena-folders-container');
        const conversations = findConversations();
        if (!hasContainer && conversations.length > 0) {
          console.log('🗂️ Sidebar recreated — re-injecting folder UI...');
          injectFolderInterface().then(() => {
            setupSidebarResizeObserver();
          });
        }
